...
flowchart TD
    subgraph DATA["<b>DATA</b>"]
        D0["<b>Training set</b>"]
        D1["<b>Common Crawl</b><br/>https://commoncrawl<br/>.org<br/>~100.000 GB"]
        D2["<b>WebText2</b><br/>https://openwebtext2<br/>.readthedocs.io<br/>~70 GB"]
        D3["<b>Wikipedia</b><br/>https://www<br/>.wikipedia.org<br/>~100 GB"]
        D4["<b>The Pile</b><br/>https://arxiv.org<br/>/abs/2101.00027<br/>~1.000 GB"]
        D5["<b>Books1/2</b><br/>unknown<br/>size/source"]
    end

    subgraph PRE["<b>PRETRAINING</b>"]
        P0["<b>Initialized/current<br/>weights</b>"]
        P1["<b>Run Flow</b>"]
        P1B["<b>Compare next token<br/>with real data</b>"]
        P2["<b>Optimize weights</b><br/>minimize<br/>training loss"]
        P3["<b>Repeat</b>"]
    ...
    subgraph FLOW["<b>FLOW</b>"]
        ...
        F1["<b>Generate input<br/>embeddings</b>"]
    ...
    subgraph MODEL["<b>MODEL</b>"]
        ...
        M1["<b>Transformer<br/>block N×</b><br/>layer normalization<br/>→ masked multi-head<br/>self attention<br/>causal: future tokens<br/>masked<br/>→ shortcut connection"]
        ...
        M3["<b>Layer normalization</b><br/>→ feed forward<br/>network<br/>GELU activation<br/>→ shortcut connection"]
        M4["<b>Outputs</b><br/>[[2.4][2.4]<br/>[2.1]...]<br/>[[-2.6][1.3]<br/>[2.1]...]<br/>[[2.0][1.8]<br/>[-1.6]...]<br/>[[2.9][1.2]<br/>[0.5]...]"]
        M5["<b>Final layer<br/>normalization</b><br/>+ output projection<br/>linear → vocabulary<br/>size"]
    ...
    subgraph POST["<b>POST-TRAINING</b>"]
        T0["<b>Weights from<br/>pretraining</b>"]
        T9["<b>Run post-training<br/>flow</b>"]
        T4["<b>Supervised<br/>fine-tuning</b><br/>instruction tuning"]
